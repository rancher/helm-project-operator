...
if kubectl get -n cattle-helm-system job/helm-delete-project-project-operator-example-dummy --ignore-not-found; then
    if ! kubectl wait --for=condition=complete --timeout="${KUBECTL_WAIT_TIMEOUT}" -n cattle-helm-system job/helm-delete-project-project-operator-example-dummy; then
        ...
        kubectl logs job/helm-delete-project-project-operator-example-dummy -n cattle-helm-system
        exit 1
    fi
fi
