...
if ! kubectl wait --for=condition=complete --timeout="${KUBECTL_WAIT_TIMEOUT}" -n cattle-helm-system job/helm-install-project-project-operator-example-dummy; then
    echo "ERROR: Helm Install Job for Example Chart never completed after ${KUBECTL_WAIT_TIMEOUT} seconds"
    kubectl logs job/helm-install-project-project-operator-example-dummy -n cattle-helm-system
    exit 1
fi
kubectl logs job/helm-install-project-project-operator-example-dummy -n cattle-helm-system
