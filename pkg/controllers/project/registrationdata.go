...
func (h *handler) getK8sRoleToSubjectsFromBindings(projectHelmChart *v1alpha1.ProjectHelmChart) (map[string][]rbac.Subject, error) {
	...
	for _, rb := range roleBindings {
		...
		k8sRole, isDefaultRoleRef := common.GetK8sRoleFromOperatorDefaultRoleName(h.opts, rb.RoleRef.Name)
		if !isDefaultRoleRef {
			continue
		}
	...
	for _, crb := range clusterRoleBindings {
		...
		k8sRole, isDefaultRoleRef := common.GetK8sRoleFromOperatorDefaultRoleName(h.opts, crb.RoleRef.Name)
		if !isDefaultRoleRef {
			continue
		}
